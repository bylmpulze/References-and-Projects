...
class MainMenuScene:
    ...
    def render(self):
        ...
        self.draw_text_centered("Drücke 1 für Singleplayer", self.font_small, (0, 100, 0), 350)
        self.draw_text_centered("Drücke 2 für Multiplayer", self.font_small, (0, 0, 200), 400)

        if self.selected_mode == "multi":
            self.draw_text_centered("Gib IP-Adresse ein:", self.font_small, (0, 0, 0), 470)
            pygame.draw.rect(self.screen, (220, 220, 220), (self.get_window_size()[0]//2 - 150, 500, 300, 40))
            ip_render = self.font_small.render(self.input_text or settings.get("multiplayer_ip"), True, (0, 0, 0))
            self.screen.blit(ip_render, (self.get_window_size()[0]//2 - 140, 505))
            self.draw_text_centered("Drücke ENTER zum Start", self.font_small, (0, 0, 0), 580)
            self.draw_text_centered("oder ESC zum Abbrechen", self.font_small, (0, 0, 0), 620)
            
            if self.error_text:
                self.draw_text_centered(self.error_text, self.font_small, (255, 0, 0), 660)
